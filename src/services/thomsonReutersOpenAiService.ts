...
export class ThomsonReutersOpenAIService {
   ...
   */
  private generateMockResponse(messages: OpenAIMessage[]): string {
    const userMessage = messages.find(m => m.role === 'user')?.content || '';
    
    // Generate contextual mock responses based on the prompt
    if (userMessage.includes('optimal capacity')) {
      return "Excellent work! Your capacity utilization is well-balanced at around 100%. Continue maintaining this steady pace while staying available for any urgent sprint items. Consider documenting your workflow for knowledge sharing with the team.";
    } else if (userMessage.includes('over capacity')) {
      return "Your current workload appears high. I recommend reviewing task priorities and considering deferring non-critical items to future sprints. Focus on completing high-priority tasks first and communicate with your team lead about workload adjustments.";
    } else if (userMessage.includes('under-utilized') || userMessage.includes('under capacity')) {
      return "You have additional capacity to take on more work. Consider picking up unassigned tasks that align with your skills, or explore opportunities for code reviews, technical debt reduction, or helping teammates with their deliverables.";
    } else if (userMessage.includes('Hello') || userMessage.includes('connection')) {
      return "Hello! Thomson Reuters OpenAI connection is working properly in development mode.";
    }
    
    return "Based on the current sprint analysis, I recommend maintaining regular communication with your team and focusing on delivering high-quality work within the planned timeline.";
  }

  /**
   * Generate contextual demo mock response for member-specific queries
   */
  private generateDemoMockResponse(
    message: string, 
    member: TeamMember, 
    ...
    const capacity = teamCapacity?.totalAvailableCapacity || 40;
    
    // Generate contextual responses based on the message content
    if (message.toLowerCase().includes('capacity') || message.toLowerCase().includes('workload')) {
      return `📊 **${memberName}'s Capacity Analysis** (Demo Mode)

**Current Status:**
• Assigned Tasks: ${assignedCount} items
• Available Capacity: ${capacity} hours this sprint
• Utilization Level: ${assignedCount > 3 ? 'High' : assignedCount > 1 ? 'Optimal' : 'Light'}
...
*This is a demo response. Configure Thomson Reuters AI for live insights.*`;
    }
    
    if (message.toLowerCase().includes('assign') || message.toLowerCase().includes('task')) {
      const suggestedTasks = unassignedTasks.slice(0, 2);
      return `🎯 **Task Assignment for ${memberName}** (Demo Mode)
...
AUTO_ASSIGN: ${suggestedTasks.map(t => `#${t.id}`).join(', ')}` : '**No unassigned tasks available**'}

*This is a demo response. Configure Thomson Reuters AI for live insights.*`;
    }
    
    if (message.toLowerCase().includes('status') || message.toLowerCase().includes('progress')) {
      const completedTasks = assignedTasks.filter(t => t.state === 'Done' || t.state === 'Completed');
      return `📈 **${memberName}'s Sprint Progress** (Demo Mode)

**Sprint Summary:**
• Total Tasks: ${assignedCount}
• Completed: ${completedTasks.length}
• In Progress: ${assignedTasks.length - completedTasks.length}
• Progress Rate: ${assignedTasks.length > 0 ? Math.round((completedTasks.length / assignedTasks.length) * 100) : 0}%

**Status:** ${completedTasks.length / assignedTasks.length > 0.7 ? 'On track' : completedTasks.length / assignedTasks.length > 0.3 ? 'Needs attention' : 'At risk'}

*This is a demo response. Configure Thomson Reuters AI for live insights.*`;
    }
    
    // Default response
    return `💬 **AI Assistant Response for ${memberName}** (Demo Mode)
...
Try asking: "What's ${memberName}'s capacity?" or "Assign tasks to ${memberName}"

*This is a demo response. Configure Thomson Reuters AI for live insights.*`;
  }

  /**
   * Process chat message about a team member and return AI response with ADO links
   */
